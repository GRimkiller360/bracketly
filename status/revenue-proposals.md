...
## 2026-08-27 — Reply to live GitHub issues about the exact bug the new Structured Output Schema Linter catches

**What it is:** Today's run added `/tools/structured-output-linter/`, a client-side linter for OpenAI/Claude/Gemini's incompatible "structured output" JSON Schema subsets — most concretely, the `"additionalProperties": false`-and-full-`required` requirement OpenAI's strict mode enforces. Unlike most prior AI-tooling launches here, which were framed around a spec being newly published, this one is framed around an *already-open, currently-active* bug pattern: a targeted search this session surfaced multiple live, unresolved GitHub issues hitting exactly this — `agno-agi/agno#2158` and `#7455` (the second specifically about OpenRouter routing to xAI/Grok and getting a 400 because a schema sanitizer injects `additionalProperties: false` unconditionally), `github/github-mcp-server#376` (OpenAI tool calls failing because generated tool schemas omit the field), `eyaltoledano/claude-task-master#1522` and `#1601` (the same failure in two different code paths of the same project), and `openai/openai-python#2024`/`#1733` (Pydantic-generated schemas tripping the same rule). This is a wider, more active bug pattern than a single library's edge case — it recurs independently across unrelated frameworks because every one of them is hand- or auto-generating a schema without knowing the provider-specific rule.

**Why it's a real fit for Bracketly specifically:** Every entry in this log follows the same recency-and-narrowness logic, but this one has an unusually strong evidence trail — six independent, currently-open GitHub issues (not closed/stale) across five different projects, all hitting the identical root cause, found via a single targeted search. That's a much higher bar of "people are visibly stuck on this right now" than most prior proposals could point to, and the fix in every case is the same one-line schema property this tool specifically flags.

**Why this agent can't do it directly:** Same standing limitation as every prior entry — this agent's GitHub App cannot fork any of these repos to open a PR, and replying to someone else's open issue on a repo Bracketly doesn't own requires a human account and judgment about whether a reply is welcome (some of these are maintainer-owned bug trackers, not general discussion spaces, so a promotional-sounding reply could reasonably be seen as unwelcome noise rather than help).

**What a human needs to do:**
1. Read the six issues above (`agno-agi/agno#2158`, `#7455`; `github/github-mcp-server#376`; `eyaltoledano/claude-task-master#1522`, `#1601`; `openai/openai-python#2024` — search each repo's issue tracker directly to confirm current state, since issue status can change) and judge, issue by issue, whether a reply linking the tool would read as genuinely helpful (e.g. "here's a free way to check whether your generated schema has this exact problem before it hits the API") versus unwelcome self-promotion on someone else's bug tracker.
2. Where it seems welcome, reply with the tool linked alongside an actual explanation of the fix (add `"additionalProperties": false` and list every property in `required`) — not a bare link.
3. No code or account changes needed on the Bracketly repo side — this is purely outbound, judgment-dependent replies to existing public issues.
