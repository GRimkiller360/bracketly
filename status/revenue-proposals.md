...
## 2026-08-28 — Submit the new MCP OAuth Client Metadata Validator to the spec's own OAuth SEP discussions

**What it is:** Today's run added `/tools/mcp-oauth-metadata-validator/`, a client-side validator for Client ID Metadata Documents (CIMD) and Protected Resource Metadata — the mechanism that replaced Dynamic Client Registration as MCP's preferred client-registration path, with RFC 7591 DCR formally deprecated in the 2026-07-28 spec. Research confirmed this is a genuinely fresh gap (adopted via SEP-991 in October 2025, with several related OAuth SEPs still being actively discussed) and that no existing free web tool validates either document type — only implementation libraries (Better Auth, Stytch, WorkOS, Auth0, Descope) and server-side CLI test suites exist. The natural venues are the `modelcontextprotocol/modelcontextprotocol` repo's Issues/Discussions around SEP-991 and the related OAuth-hardening SEPs, where people actively implementing this are already asking questions per the research.

**Why it's a real fit for Bracketly specifically:** Same recency-and-narrowness logic behind every prior AI-tooling submission proposal here, but pointed at a part of the MCP ecosystem this site hasn't targeted yet — every previous MCP tool here (schema, migration, tasks, registry, apps) validates a message or manifest shape; this one validates an OAuth security artifact, a distinct and currently-thin sub-niche. The exact-string client_id matching gotcha and the well-known-URL-insertion-point mistake (both concrete, verified-in-the-wild issues per the build research: python-sdk #1049, atlassian-mcp-server #148, toolhive #2197) give any submission a specific, non-generic hook rather than reading as self-promotion.

**Why this agent can't do it directly:** Same standing limitation as every prior entry — this agent's GitHub App cannot fork `modelcontextprotocol/modelcontextprotocol` to open a PR against any docs/resources list, and replying in someone else's GitHub Discussion/Issue requires a human account and per-community judgment about self-promotion norms.

**What a human needs to do:**
1. Check `github.com/modelcontextprotocol/modelcontextprotocol` Issues/Discussions around SEP-991 and the related OAuth-hardening SEPs for existing questions about validating a hand-authored CIMD or Protected Resource Metadata document, and reply with the tool where genuinely relevant.
2. Consider a short, non-spammy post in an MCP-focused Discord/subreddit's auth-discussion threads, framed around a specific gotcha (the exact-string client_id match, or the well-known URL insertion point) rather than generic self-promotion.
3. No code or account changes needed on the Bracketly side — this is purely outbound promotion of an already-shipped tool.
